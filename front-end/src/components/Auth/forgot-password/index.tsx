...
const ForgotPasswordIndex = () => {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (!emailParams) {
        ...
        handleReplaceRoute(
          pathname + "?" + createQueryString("email", inputEmail),
        );
      }
  ...
  const handleSubmitOtp = async (data: string[]) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate.replace(`/auth/forgot-password/change-password`);
      }, 1000);
    } catch (error: any) {
      const backendMsg =
        error?.response?.data?.message ||
      ...
      <div>
        <>
          {!emailParams ? (
            ...
            <>
              {isMobile ? (
                <div className="w-full h-screen items-center flex justify-center shadow-2xl bg-gray-100">
                  <MobileFormItem>
                    <h1 className="text-center font-semibold py-5 tracking-wider capitalize">
...
const MobileInputOtp = styled(InputOTP)``;

const MobileFormItem = styled(Form.Item)`
